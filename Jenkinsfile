...
pipeline {
    ...
    stages{
        ...
        stage("Publishing covarage reports"){
            steps{
                publishHTML([allowMissing: false, alwaysLinkToLastBuild: false, keepAll: false, reportDir: 'target/scala-2.11/scoverage-report', reportFiles: 'index.html', reportName: 'SBT Coverage report', reportTitles: ''])
            }
        }
    }
}